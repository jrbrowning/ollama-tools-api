...
def check_ollama_installed():
    if shutil.which("ollama") is None:
        ...
        log("➡️  Install from https://ollama.com/download")
        sys.exit(1)
...
def start_ollama():
    if _daemon_running():
        log("ℹ️ Ollama daemon already running on :11434")
        return

    log("🚀 Starting Ollama daemon on :11434 …")
    env = os.environ.copy()
...
def wait_for_ready(seconds: int = 10):
    log("⏳ Waiting for Ollama to answer …")
    import requests
    ...
    for _ in range(seconds):
        try:
            ...
            pass
        time.sleep(1)
    log("❌ Ollama did not start in time.")
    sys.exit(1)
...
def pull_or_verify_model(model: str, online: bool):
    if online:
        ...
        log(f"📴 OFFLINE MODE: Verifying {model} is cached …")
...
def up():
    ...
    if model in output:
        log(f"✅ Model '{model}' already exists — ready.")
    else:
        log(f"📡 Model '{model}' not found.")
        if os.path.exists(SYSTEM_PROMPT_PATH):
            log("🧱 Building with system prompt …")
    ...
    log(f"🏁 Model '{model}' available at http://localhost:11434")


def rebuild():
    """
    Full reset: stop daemon ➜ remove model & base ➜ start fresh ➜ recreate/pull.
    """
    config = load_config()
    model = config.LOCAL_GPU
    ...
    # 4. Recreate or pull
    if os.path.exists(SYSTEM_PROMPT_PATH):
        log("🧱 Rebuilding with system prompt …")
        modelfile = build_modelfile(model)
        subprocess.check_call(["ollama", "create", model, "-f", modelfile])
    else:
        pull_or_verify_model(model, config.ENABLE_LOCAL_GPU_MODEL)

    log(f"🏁 Rebuild complete — '{model}' ready at http://localhost:11434")


def stop():
    if os.path.isfile(PID_FILE):
        log("🛑 Stopping Ollama daemon …")
        try:
        ...
        log("ℹ️ No PID file; daemon not running.")


def print_help():
    print(
        """
run_local_gpu.py — manage a local Ollama GPU instance
