...
check_ollama_backend() {
    ...
    if curl -s --connect-timeout $TIMEOUT "http://$container:11434/api/tags" >/dev/null; then
        echo "✅ $container container is reachable."
    else
        echo "❌ $container container is NOT reachable on port 11434."
    fi
}
...
check_chat_endpoint() {
    local prefix=$1
    local model=$2
    echo "💬 Testing chat endpoint for: $prefix (model: $model) ..."
...
# --- Start Check ---
echo "🔧 Running model container health checks..."
...
echo ""
echo "🔧 Running FastAPI chat endpoint tests..."
